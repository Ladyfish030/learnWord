  ...
  <div class="background">
    <div class="login">
      ...
      <div class="main">
        ...
        <div class="button" v-if="reg == 'selected'" @click="register">
          注册
        </div>
      </div>
    </div>
...
export default {
  ...
  methods: {
    ...
    login() {
      ...
      if (username.length < 1) return alert('用户名不能为空')
      if (password.length < 6) return alert('密码至少需要6位')
      axios({
        url: 'http://47.113.186.74/api/login.html',
        headers: {
    ...
    register() {
      ...
      // this.$axios({
      axios({
        url: 'http://47.113.186.74/api/reg.html',
        headers: {
